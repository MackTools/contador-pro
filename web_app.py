...
<style>
    ...
    /* Prevenir que el sidebar se mueva */
    ...
    /* Ocultar X de colapso con mas especificidad */
...
<!-- Script para mantener sidebar abierta -->
<script>
    setInterval(function() {
        ...
        // Ocultar botones de colapso
        var collapseBtns = parent.document.querySelectorAll('[data-testid="stSidebarCollapseButton"], [kind="header"]');
...
def pagina_login():
    ...
    with col2:
        ...
        tab1, tab2, tab3 = st.tabs(["Iniciar sesion", "Registrarse", "Recuperar"])
        ...
        with tab2:
            with st.form("registro_form"):
                ...
                pass_reg2 = st.text_input("Confirmar", type="password")
                
                if st.form_submit_button("Crear cuenta", use_container_width=True):
                    if not nombre or not email_reg or not pass_reg:
                    ...
                    elif email_reg in st.session_state.usuarios:
                        st.error("Ya registrado")
                    else:
                        st.session_state.usuarios[email_reg] = {"password": pass_reg, "nombre": nombre}
                        st.success("Cuenta creada! Inicie sesion")
        
        with tab3:
            st.caption("Ingrese su correo para recibir un codigo")
            email_rec = st.text_input("Correo", key="rec_email")
            ...
            if st.button("Enviar codigo", use_container_width=True):
                if email_rec in st.session_state.usuarios:
                    ...
                    st.success("Codigo enviado!")
                    st.info(f"Codigo: **{codigo}**")
                    st.session_state.tmp_email = email_rec
                else:
            ...
            if "tmp_email" in st.session_state:
                ...
                if st.button("Cambiar", use_container_width=True):
                    email = st.session_state.tmp_email
                    info = st.session_state.codigos_recup.get(email, {})
                    
                    if datetime.now().timestamp() > info.get("expira", 0):
                        st.error("Codigo expirado")
                    elif codigo_input == info.get("codigo", ""):
                        if len(nueva_pass) >= 6:
                            st.session_state.usuarios[email]["password"] = nueva_pass
                            del st.session_state.codigos_recup[email]
                            del st.session_state.tmp_email
...
def pagina_app():
    ...
    with st.sidebar:
        ...
        with st.expander("Nuevo proyecto", expanded=False):
            nombre_nuevo = st.text_input("Nombre", key="new_name", placeholder="Nombre proyecto")
            tipo = st.selectbox("Plantilla", ["Libro Diario", "Balanza", "Cuentas T"], key="new_type")
        ...
        with st.expander("Importar Excel", expanded=False):
            archivo = st.file_uploader("Archivo", type=["xlsx", "xls"], key="imp", label_visibility="collapsed")
            if archivo:
                try:
        ...
        for nombre in list(st.session_state.proyectos.keys()):
            n = len(st.session_state.proyectos[nombre]["datos"])
    ...
    if st.session_state.proyecto_actual:
        ...
        st.caption(f"Creado: {info.get('fecha', '')} | Modificado: {info.get('modificado', '')} | Filas: {len(df)}")
        
        if not df.empty:
            col_config = {}
            for col in df.columns:
                if any(x in col.lower() for x in ["debe", "haber", "saldo", "monto"]):
                    col_config[col] = st.column_config.NumberColumn(col, format="$ %,.2f")
            
            edited_df = st.data_editor(
                df, num_rows="dynamic", use_container_width=True,
                height=380, column_config=col_config if col_config else None,
                key=f"ed_{nombre}", hide_index=True
            )
        ...
        if st.session_state.show_chart and "Debe" in edited_df.columns and "Haber" in edited_df.columns:
            st.divider()
        ...
        with cr1:
            if st.button("Balance General", use_container_width=True):
                ...
                st.info(f"P+C: ${pasivos+capital:,.2f} | Dif: ${activos-(pasivos+capital):,.2f}")
        
        with cr2:
            if st.button("Est. Resultados", use_container_width=True):
                ingresos = gastos = 0
                for _, row in edited_df.iterrows():
                    ...
                    if any(p in desc for p in ['ingreso', 'venta', 'servicio']):
                        ingresos += haber
                    elif any(p in desc for p in ['gasto', 'costo', 'compra']):
                        gastos += debe
        ...
        <div style="text-align: center; padding: 60px 20px;">
            ...
            <p style="color: #64748b;">Sistema de gestion contable</p>
            <br>
            <p>Seleccione un proyecto del menu lateral</p>
        </div>
        """, unsafe_allow_html=True)
